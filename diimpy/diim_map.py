...
def get_solar_position(time_utc,lats,lons,folder='./',dateformat='%Y%m%d-%H:%M:%S',save=True,read=True,filename_date=True,filename=''):

    if filename_date == True:
        filename = 'zenith_'+datetime.strftime(time_utc + timedelta(hours=1),dateformat )+'.npy'
        
    if os.path.exists(folder + '/' + filename) & (read == True):
        zenith_ = np.load(folder + '/' + filename)
        return zenith_
    ...
    if save == True:
        np.save(folder + '/'+filename,zenith_angle_)
    return zenith_angle_


@njit
def integrate_par(Edif,Edir,wl,irange,ilen,constant_,n_hour_average=10):
...
def local_initial_conditions_nn(F_model,constant,data,precision = float32,my_device = 'cpu',rearange_needed=True):
    ...
    model_NN.load_state_dict(tload(MODEL_HOME + '/settings/reproduce_dukiewicz/VAE_model/model_second_part_chla_centered.pt'))
    model_NN.eval()

    if rearange_needed == True:
        X,Y = data
        data_rearanged = empty((X.shape[0],1,17),dtype=precision)
    ...
    if tisnan(mu_z).any():
        mu_z[targwhere(tisnan(mu_z))] = trand(mu_z[targwhere(tisnan(mu_z))].shape,dtype=precision)
    mu_z[mu_z > 3.5] = tensor(3.5,dtype = precision)
...
def train_loop(data_i,model,loss_fn,optimizer,N,kind='all',num_days=1,my_device = 'cpu',constant = None,perturbation_factors_ = None,calc_kd=True,calc_bbp=True):
    ...
    K_x = empty((len(parameters_eval),5,3),dtype=s_e_inverse.dtype)
    for i in range(len(parameters_eval)):
        evaluate_model.X = X[i].unsqueeze(0)
        K_x[i] = tjacobian(evaluate_model.model_der,inputs=(parameters_eval[i].unsqueeze(0)))[0,:,0,0,:]
    
    S_hat = tinverse( ttranspose(K_x,1,2) @ ( s_e_inverse @ K_x ) + s_a_inverse  )
    
    X_hat = empty((len(parameters_eval),6),dtype=s_e_inverse.dtype)
    ...
    if calc_kd:
        kd_hat = empty((len(parameters_eval),10),dtype=s_e_inverse.dtype)
        kd_values = evaluate_model.kd_der(parameters_eval)

        kd_derivative = empty((len(parameters_eval),5,3),dtype=s_e_inverse.dtype)
        for i in range(len(parameters_eval)):
            evaluate_model.X = X[i].unsqueeze(0)
            kd_derivative[i] = tjacobian(evaluate_model.kd_der,inputs=(parameters_eval[i].unsqueeze(0)))[0,:,0,0,:]
    ...
    if calc_bbp:
        ...
        bbp_hat = empty((len(parameters_eval),6),dtype=s_e_inverse.dtype)
        bbp_values = evaluate_model.bbp_der(parameters_eval)
        bbp_derivative = empty((len(parameters_eval),3,3),dtype=s_e_inverse.dtype)
        for i in range(len(parameters_eval)):
            evaluate_model.X = X[i].unsqueeze(0)
            bbp_derivative[i] = tjacobian(evaluate_model.bbp_der,inputs=(parameters_eval[i].unsqueeze(0)))[0,:,0,0,:]
